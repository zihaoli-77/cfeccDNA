...
eccDNA_sbpm <- function(bamfile){
  ...
  param <- ScanBamParam(flag = scanBamFlag(isPaired = NA, isProperPair = NA, isUnmappedQuery = NA,
                        ...
                        what = "seq")#设置读入bam文件的参数
  primary <- readGAlignments(bamfile, param = param,use.names = T) #读bam文件
...
for (i in seq_along(input_files)) {
  ...
  output_file <- file.path("/public/home/hpc216085/eccDNA/sample/split.sorted/4mersBPM", input_files[[i]]$output_file)
