...
eccDNA_bpm <- function(bamfile){
  ...
  gr1 <- GRanges(seqnames = pA.chr,ranges = IRanges(start = end-1,end = end+2))
  gr2 <- GRanges(seqnames = pB.chr,ranges = IRanges(start = start-2,end = start+1))
  ...
  bpm<- as.data.frame(prop.table(table(com)))
  names(bpm) <- c('motif','frequency')
  number_motif<-"4"
  ...
  BPM<-merge(empty.df,bpm,by='motif',all.x=TRUE)
  BPM[is.na(BPM)] <- 0
  return(BPM)
...
input_files <- lapply(file_list_pri, function(file) {
  ...
  list(input_file = file,
       output_file = paste0(sample_name, "_BPM", ".txt"))
})

for (i in seq_along(input_files)) {
  result <- eccDNA_bpm(input_files[[i]]$input_file)
